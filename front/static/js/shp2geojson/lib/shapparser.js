...
SHPParser.prototype.parseShape = function(dv, idx, length) {
    ...
    switch (shape.type) {        
        case 11: // PointZ (X, Y, Z, M)
            shape.type = SHP.POINT;
            break;
        case 13: // PolylineZ
            shape.type = SHP.POLYLINE;
            break;
        case 15: // PolygonZ  
            shape.type = SHP.POLYGON;
            break;
        default:
            break;
    }
...
DBFParser.prototype.parse = function(arrayBuffer,src,response,encoding) {
    ...
    offset = (encoding.match(/GBK/i)) ? 2 : 3;

    o.fileName = src;
    o.version = dv.getInt8(idx, false);//0

    idx += 1;
    o.year = dv.getUint8(idx) + 1900;//1
    idx += 1;
    o.month = dv.getUint8(idx);//2
    idx += 1;
    o.day = dv.getUint8(idx);//3
    idx += 1;

    o.numberOfRecords = dv.getInt32(idx, true);//4
    idx += 4;
    o.bytesInHeader = dv.getInt16(idx, true);//8
    idx += 2;
    o.bytesInRecord = dv.getInt16(idx, true);//10
    idx += 2;
    //reserved bytes
    idx += 2;
    o.incompleteTransation = dv.getUint8(idx);//14
    idx += 1;
    o.encryptionFlag = dv.getUint8(idx);//15
    idx += 1;
    // skip free record thread for LAN only
    idx += 4;
    // reserved for multi-user dBASE in dBASE III+
    idx += 8;
    o.mdxFlag = dv.getUint8(idx);//28
    idx += 1;
    o.languageDriverId = dv.getUint8(idx);//29
    idx += 1;
    // reserved bytes
    idx += 2;//32
    ...
    while (responseHeader.length > 0) {
        ...
        charString.push(responseHeader.slice(0, z).replace(/\0/g, ''));
        responseHeader = responseHeader.slice(32 +z- count, responseHeader.length);
    }
    //2.获取字段类型
    while (true) {
        var field = {},
